    ...
    static async signUp(rawData) {
        ...
        if (!verifier) {
            return {
                response: false,
                message:
                    "Please receive and resend the Email verification code.",
                data: null,
            };
        }

        if (Number(verifier.verify_code) !== rawData.email_verify) {
            return {
                response: false,
        ...
        //check invite code
        // NOTE: commenting the inviter check as it's optional for signing up
        ...
        rawData.invite_code = this.makeRandomString(4);
        ...
        const body = i18n.__({
            phrase: "Your MGL Exchange account has been created sucessfully. Thank you.",
            locale: rawData.locale || "En",
        });
