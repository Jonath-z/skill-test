...
class P2PService {
    ...
    static async createP2P(rawData) {
        try {
            ...
            if (
                rawData.price_usdt === 0 &&
                rawData.price_mgl === 0 &&
                rawData.price_busd === 0
            ) {
                return {
                    response: false,
            ...
            if (
                rawData.amount_mgl === 0 &&
                rawData.amount_usdt === 0 &&
                rawData.amount_busd === 0
            ) {
                return {
                    response: false,
            ...
            if (
                rawData.currency !== "USDT" &&
                rawData.currency !== "MGL" &&
                rawData.currency !== "BUSD"
            )
                return {
                    response: false,
                    message: "Currency type is wrong!",
                    data: null,
                };
            if (!rawData.payment_method)
                return {
                    response: false,
                    message: "Payment method is wrong!",
                    data: null,
                };
    ...
    static async updateP2P(rawData, p2pId) {
        try {
            ...
            if (!p2p) {
                return {
                    response: false,
                    message: "That P2P does not exist.",
                    data: null,
                };
            }
            if (
    ...
    static async getAllP2P({ page_num, per_page }) {
        try {
            ...
            if (result.length === 0) {
                return {
                    response: false,
                    message: "There is no registered P2P.",
                    data: null,
                };
            }
            if (page_num && per_page) {
                const filterResult = result.filter(
                    (item, index) =>
                        index >= per_page * (page_num - 1) &&
                        index < per_page * page_num,
                );
                const newFilterResult = filterResult.map((data) => {
                    const arr = data.profile_pic.split(".");
                    data.profile_pic =
                        `data:image/${arr[arr.length - 1]};base64,` +
                        fs.readFileSync(data.profile_pic, "base64");
                    return data;
                });
                return {
                    response: true,
                    message: "Success",
                    data: newFilterResult,
    ...
    static async getOneP2P(p2pId) {
        try {
            ...
            result.profile_pic =
                `data:image/${arr[arr.length - 1]};base64,` +
                fs.readFileSync(result.profile_pic, "base64");
