...
const CollapseData = [
    { title: "Products", data: [{ icon: "", subtitle: "" }] },
    {
        ...
        data: [
            {
                icon: <BsCreditCard2Front size={20} className="inline mx-2" />,
                subtitle: "Credit/Debit Card",
            },
            {
                icon: <BsPeople size={20} className="inline mx-2" />,
    ...
    return (
        ...
        >
            <div className="flex flex-row-reverse">
                <a onClick={() => props.setSubmenu(false)}>
            ...
            <div className="text-center border-b-2 border-gray-200 mt-4 pb-4">
                {localStorage.getItem("userInfo") ? (
                    <Link to="/" onClick={logout}>
                        <p className="text-lg font-bold">{t("Log out")}</p>
                    </Link>
                ) : (
                    <Link to="/login">
                ...
                <Link to="/register">
                    <button className="w-full myButton myBule text-white px-12 mt-4">
                        {t("Register Now")}
                    </button>
                </Link>
            </div>
                            ...
                            {item.data.map((item, idx) => (
                                <div>
                                    <button
                                        ...
                                        className="grayButton w-full text-left px-2 py-2 rounded-lg"
                                    >
                                        {item.icon} {t(item.subtitle)}
                                    </button>
                                </div>
                            ))}
                        </div>
            ...
            <div className="flex">
                ...
                <button className="flex-grow myButton  bg-blue-100  text-sm m-2">
                    {" "}
                    USD{" "}
                </button>
            </div>
