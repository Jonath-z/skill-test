      ...
      <div className="w-full lg:w-7/12 flex flex-col mr-0 lg:mr-8">
        <div className="w-full flex justify-between flex-col lg:flex-row">
          <div className="flex items-center">
            <div className="font-bold flex cursor-pointer text-2xl items-center">
              <img className="h-14 w-14 rounded-full" src={item.profile_pic} />
              &nbsp;{item.seller_name}
            </div>
            <div className="my-1 bg-green-200 rounded-lg font-bold flex items-center justify-center py-1 px-4 ml-4 text-xs">
              {t("MGL Verified")}
            </div>
          </div>
          <div className="w-8/12 lg:w-3/12 my-1">
            <SelectComponent
              optionList={contactList}
              groupName="Ways to connect"
        ...
        <div className="w-full flex justify-between items-start lg:items-center flex-col lg:flex-row my-1">
          ...
          <div className="text-xs p-2 bg-green-100 rounded-lg my-1">
            {t(item.payment_method.replaceAll(",", "/"))}
          </div>
          <div></div>
        </div>
        <div className="w-full flex flex-col">
          ...
          <div
            dangerouslySetInnerHTML={{ __html: item.terms_conditions }}
          ></div>
        </div>
      </div>
