...
function AdminSendTokenRow(props) {
    ...
    const changeEmail = (val) => {
        if (
            val.includes("@") &&
            val.includes(".") &&
            val.length - val.lastIndexOf(".") > 2 &&
            val.length - val.lastIndexOf(".") < 5
        ) {
            setEmail(val);
        }
    };
    ...
    useEffect(() => {
        if (email.length > 0) {
            ...
            axios
                .get(`${serverUrl}wallets/getwalletbyemail/${email}`, {})
                .then((response) => {
    ...
    useEffect(() => {
        (async () => {
            if (props.canDo === true) {
                ...
                setAuthToken(localStorage.jwtToken);
                axios
                    .post(serverUrl + "wallets/sendtoken", {
                        publicKey: publicKey,
                        destination: address,
                        token: tokenAddress,
                        amount: amount,
        ...
        <React.Fragment>
            ...
            <Col span={2}>
                <Select
                    ...
                    className="mt-2 bg-gray-200 w-2/2"
