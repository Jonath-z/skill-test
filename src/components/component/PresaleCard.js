...
function PresaleCard(props) {
    ...
    const timeoffset =
        (currentLocalTime.getTimezoneOffset() + 60 * 11) * 60 * 1000;
    ...
    useEffect(() => {
        ...
        let remainTime = Math.floor(
            (launchTime.getTime() - timeoffset - currentTime.getTime()) / 1000,
        );
    ...
    const formatTime = (timestamp) => {
        ...
        let mins = Math.floor(
            (timestamp - days * 3600 * 24 - hours * 3600) / 60,
        );
        let seconds = Math.floor(
            timestamp - days * 3600 * 24 - hours * 3600 - mins * 60,
        );
        ...
        >
                ...
                <div>
                    {props.state === 0 ? (
                        <button className="myButton2 bg-yellow-200 px-4 py-0  rounded-lg ">
                            <BsDot size={30} className="inline" />
                            In Progress
                        </button>
                    ) : props.state === 1 ? (
                        ...
                        <button className="myButton2 bg-gray-300 px-4 py-0  rounded-lg ">
                            <RiLockPasswordLine size={30} className="inline" />
                            Canceled
                        </button>
                    )}
                </div>
            ...
            <div className="flex justify-between mt-8">
                ...
                <a
                    target="_blank"
                    href={props.link}
                    className="myButton text-white rounded-lg px-6  myBule font-bold text-sm xl:text-lg"
                >
                    View IEO
                </a>
            </div>
