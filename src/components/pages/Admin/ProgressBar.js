...
function P2PTableView() {
  ...
  const update = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        } else openNotification("Failed", response.data.message, false, null);
      })
      .catch((err) => {
  ...
  const percentSold = useCallback(() => {
    ...
    if (IEOData.id)
      return `${((IEOData.raised_amount / IEOData.total_supply) * 100).toFixed(
        5
      )} %`;
    return "0 %";
  }, [IEOData]);
  return (
    <Content style={{ margin: "24px 16px 0" }}>
      <div
        className="text-lg text-right  site-layout-background full-height"
        style={{ padding: 24, minHeight: 360 }}
      >
        <p className="text-left font-bold my-4">MGL</p>
        <Row>
          <Col span={8}>Current Sold :</Col>
          <Col span={7} offset={1}>
            <input
        ...
        <Row className="mt-8">
          <Col span={8} offset={8}>
            <button
              className="mx-3 myButton  myBule text-white w-full text-sm"
              onClick={update}
            >
              {t("Update")}
            </button>
