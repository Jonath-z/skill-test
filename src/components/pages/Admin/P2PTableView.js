...
function P2PTableView() {
        ...
        form.validateFields()
            .then((values) => {
                const tradeData = {
                    ...
                    amount_usdt:
                        values.currency === "USDT" || !values.currency
                            ? values.amount * 1
                            : 0,
                    amount_mgl:
                        values.currency === "MGL" ? values.amount * 1 : 0,
                    amount_busd:
                        values.currency === "BUSD" ? values.amount * 1 : 0,
                    payment_method: values.payment_method
                        ? values.payment_method.toString()
                        : "Cash",
                    price_usdt: values.price_usdt,
                    price_mgl: values.price_mgl,
                    price_busd: 0,
                    terms_conditions: values.terms_conditions
                        ? values.terms_conditions.replaceAll(/\n/g, "<br/>")
                        : "",
                ...
                const url = addUpdateStatus
                    ? `${SERVER_URL}p2p/id/${selectedIndex}`
                    : `${SERVER_URL}p2p/create`;
                axios.post(url, tradeData).then((res) => {
    ...
    const deleteItem = () => {
        axios
            .delete(`${SERVER_URL}p2p/id/${selectedIndex}`, {})
            .then((response) => {
            ...
            form.setFieldsValue({
                ...
                amount:
                    selectedData.currency === "USDT"
                        ? selectedData.amount_usdt
                        : selectedData.currency === "MGL"
                          ? selectedData.amount_mgl
                          : selectedData.amount_busd,
    ...
    return (
        <Content style={{ margin: "24px 16px 0" }}>
            <div
                className="site-layout-background full-height"
                style={{ padding: 24, minHeight: 360 }}
            >
                <Row>
                    <Col span={24} className="horizont-space general-padding">
                        <span className="item-display bold-text">
                            Total :&nbsp;&nbsp; {totalCount}
                        </span>
                        <span>
                            <Button
                ...
                >
                    ...
                    <Column
                        ...
                        render={(text, record) =>
                            record.payment_method.replaceAll(",", "/")
                        }
                        key="method"
                    />
                    <Column
                    ...
                    <Column
                        title="Facebook"
                        render={(text, record) => (
                            <div className="w-40">{record.facebook_link}</div>
                        )}
                        key="facebook"
                    />
                    <Column
            ...
            >
                ...
                >
                    ...
                    <Form.Item name="currency">
                        <Select>
                            <Select.Option value="USDT" key="usdt">
                                USDT
                            </Select.Option>
                            <Select.Option value="BUSD" key="busd">
                                BUSD
                            </Select.Option>
                            <Select.Option value="MGL" key="mgl">
                                MGL
                            </Select.Option>
                        </Select>
                    </Form.Item>
                    ...
                    <Form.Item name="payment_method">
                        <Select mode="multiple">
                            <Select.Option value="Bank transfer" key="usdt">
                                Bank transfer
                            </Select.Option>
                            <Select.Option value="Cash" key="mgl">
                                Cash
                            </Select.Option>
                        </Select>
                    </Form.Item>
