...
function UserTableView() {
    ...
    const handleTableChange = (pageInfo, filters, sorter) => {
        ...
        axios
            .get(
                `${SERVER_URL}users/id/${pageInfo.current}/${pageInfo.pageSize}`,
                {},
            )
            .then((response) => {
    ...
    const deleteUser = () => {
        ...
        axios
            .delete(`${SERVER_URL}users/id/${userToDel}`, {})
            .then((response) => {
                console.log(response.data);
                if (response.data.response) {
                    axios
                        .get(
                            `${SERVER_URL}users/id/${currPageNum}/${itemPerPage}`,
                            {},
                        )
                        .then((response) => {
                            if (response.data.response) {
                                setTotalUser(response.data.data.totalNumber);
                                setPagination({
                                    total: response.data.data.totalNumber,
                                });
                                setDisplayData(response.data.data.users);
                            }
                        });
    ...
    useEffect(() => {
        ...
        axios.get(SERVER_URL + "users/id/1/10", {}).then((response) => {
            if (response.data.response) {
                setTotalUser(response.data.data.totalNumber);
                setPagination({ total: response.data.data.totalNumber });
                setDisplayData(response.data.data.users);
